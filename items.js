...
fetch(`https://pokeapi.co/api/v2/item?limit=${MAX_POKEMON}`)
...
async function fetchPokemonDataBeforeRedirect(id) {
  try {
    ...
    console.error("Failed to fetch Item data before redirect");
  }
}
...
function displayPokemons(pokemon) {
  ...
  pokemon.forEach((pokemon) => {
    ...
    listItem.innerHTML = `
        ...
        <div class="img-wrap">
            <img src="https://raw.githubusercontent.com/pokeapi/sprites/master/sprites/items/${pokemon.name}.png" alt="${pokemon.name}" />
        </div>
        <div class="name-wrap">
    ...
    listItem.addEventListener("click", async () => {
      ...
      if (success) {
        window.location.href = `./items-detail.html?id=${pokemonID}`;
      }
    });
